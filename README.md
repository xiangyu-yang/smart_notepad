...
- **🧩 工程化代码质量**：仓储模式事务化写入、服务层按单一职责拆分、Zustand 多 store 边界清晰、TypeScript 严格模式下零 `any` 与零 `require()`
  ...
  <br />
- **🗂️ 文件夹树管理**：邻接表 `parent_id` 自引用 CASCADE 支持无限嵌套；记事与文件夹均支持 HTML5 原生拖拽——拖到目标文件夹移入、拖到空白处移回根级；移动后端递归 CTE 循环检测，前端 `isInSubtree` 预检 + 后端兜底双重防护，杜绝 parent\_id 成环；删除文件夹递归 CTE 收集后代并级联清理记事与聊天历史
